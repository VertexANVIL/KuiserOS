...
let
  inherit (tools.router) resolvePeers;
  ...
  buildPeerProtocol = data: protocol:
    ...
    imapAttrsToList
      (i: n: v:
        let
          name = if i == 0 then "bgp_${data.peerName}_${protocol}" else
  ...
  buildPeer = peer:
    ...
    in
    assert (assertMsg (data.hasv4 || data.hasv6) "failed to resolve any addresses for neighbor ${data.neighbor.name}");
    ...
    let
      ...
      fullRoutes = block.routes.static ++
        ...
        (optional (block.default != null) (defaults // {
          address = if (v == 6) then "::0" else "0.0.0.0";
          prefixLength = 0;
          via = block.default;
          prefsrc = "@underlay";
        }));
    in
    buildStaticRoutes fullRoutes v;

  # Builds a BIRD network set from a list of address blocks
  buildNetworkSet = addr: v:
    let
  ...
  ourPeer = tools.internalBgpPeers.${eidolon.network};
  birdConf = ''
