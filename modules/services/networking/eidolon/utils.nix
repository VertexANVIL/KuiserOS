...
let
  ...
  regions = foldl'
    (prev: cur:
      ...
      in
      prev // {
        ${name} = list ++ (optional (name != null) cur);
      })
    { }
    (attrValues nodes);
      ...
      resolvePeerBgp = input: (
        let
          ...
          peer =
            let
              ...
              found = mapAttrsToList (_: x: x) (filterAttrs
                (name: peer:
        ...
        in
        {
          ...
          neighbor = {
            ...
            v4addrs = defaultSetAttrs (neighborKeyToAttrs neighborAttrs "v4addrs") neighborDefaultAttrs peer;
            v6addrs = defaultSetAttrs (neighborKeyToAttrs neighborAttrs "v6addrs") neighborDefaultAttrs peer;
          };
        }
      );
...
in
{
  inherit router regions underlay underlayAddr parsePrefixRoute;
}
