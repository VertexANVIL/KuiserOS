...
rec {
  ...
  genPkgs = { inputs, base ? nixpkgs }:
    let
      ...
      pkgs = (eachDefaultSystem (system:
        ...
        in
        { pkgs = pkgImport base overlays system overrides.unfree; }
      )).pkgs;
    in
    pkgs;
  ...
  nixosLib = { inputs, pkgs, home ? false, ... }:
    let
      inherit (inputs) self;
        ...
        mkProfile =
          ...
          let
            ...
            requires = mapAttrs
              ...
              sources;
          in
          (filterAttrs (n: v: n != "requires") attrs) // {
  ...
  nixosLibHm = { inputs, pkgs }:
    let
      lib = nixosLib { inherit inputs pkgs; home = true; };
    in
    import (inputs.home + "/modules/lib/stdlib-extended.nix") lib;
  ...
  mkBaseRepo = all@{ inputs, _root ? null, ... }:
    let
  ...
  mkRepo =
      ...
      repo =
        let
          ...
          shallowMerged = recursiveMergeAttrsWithNames
            [ "nixosModules" "overlays" "packages" ]
            (a: b: a // b) [ parent local ];
          deepMerged = recursiveMergeAttrsWithNames
            [ "lib" "_internal" ]
            (a: b: recursiveMerge [ a b ]) [ parent local ];
        in
        (shallowMerged // deepMerged) // {
    ...
    in
    recursiveUpdate repo hostOutputs;
  ...
  mkNixosSystem =
    ...
    , # The flake inputs
      nodes ? { }
    , # Set of nodes to allow inter-node resolution

      name
    , # The hostname of the host
      config
    , # The base configuration for the host
      system
    , # Target system to build for
      pkgSets
    , # The compiled package sets
    }:
    let
      inherit (inputs) self;
  ...
  mkHomes =
    ...
    in
    recursiveMerge (mapAttrsToList
      (host: _: recImportDirs {
        dir = root + "/users";
